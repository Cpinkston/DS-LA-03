...
Visualizer function for the TSP graphs and paths taken
*/
...
public class Plot2D {
    ...
    PlotPanel plotPanel;
    ArrayList<double[]> cityList = new ArrayList<double[]>();
    ...
    // Converts the "ArrayList of an Array of Doubles to something less retarded."
    
    private JPanel getContent(ArrayList<double[]> cityList) {
        //double[] x = getData(-MAX, MAX);
        //double[] y = getData(-MAX, MAX);
        double[] x = new double[cityList.size()];
        double[] y = new double[cityList.size()];
        for (int i=0; i<cityList.size(); i++){
            double[] temp = cityList.get(i);
            x[i] = (Double) temp[1];
            y[i] = (Double) temp[2]; 
        }
    ...
    // YASIN SAY: HERE BE THE MAIN FUNCTION. 
    public static void main(String[] args) {
        System.out.println("You cannot call this directly. Call using TSPTester.java --Yasin");
        return;
    }
    public static void run(ArrayList<double[]> cityList) {
        Plot2D test = new Plot2D();
        JFrame f = new JFrame();
        f.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
        f.add(test.getContent(cityList));
        f.setSize(400,400);
        f.setLocation(50,50);
        f.setVisible(true);
        return;
